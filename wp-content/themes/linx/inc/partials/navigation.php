    ...
    <?php if ( ! empty( $prev_post ) ) : ?>
      <div class="nav previous">
        <?php if ( has_post_thumbnail( $prev_post->ID ) ) :
          if ( wp_get_attachment_image_srcset( get_post_thumbnail_id( $prev_post->ID ), $image_size ) ) : ?>
            <img class="lazyload" data-srcset="<?php echo esc_attr( wp_get_attachment_image_srcset( get_post_thumbnail_id( $prev_post->ID ), $image_size ) ); ?>" data-sizes="auto">
          <?php else :
            $image = wp_get_attachment_image_src( get_post_thumbnail_id( $prev_post->ID ), $image_size ); ?>
    ...
    <?php if ( ! empty( $next_post ) ) : ?>
      <div class="nav next">
        <?php if ( has_post_thumbnail( $next_post->ID ) ) :
          if ( wp_get_attachment_image_srcset( get_post_thumbnail_id( $next_post->ID ), $image_size ) ) : ?>
            <img class="lazyload" data-srcset="<?php echo esc_attr( wp_get_attachment_image_srcset( get_post_thumbnail_id( $next_post->ID ), $image_size ) ); ?>" data-sizes="auto">
          <?php else :
            $image = wp_get_attachment_image_src( get_post_thumbnail_id( $next_post->ID ), $image_size ); ?>
            <img class="lazyload" data-src="<?php echo esc_url( $image[0] ); ?>">
          <?php endif;
        endif; ?>
